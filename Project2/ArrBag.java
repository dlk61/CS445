...
public class ArrBag<T>
{
  ...
  // APPENDS NEW ELEM AT END OF LOGICAL ARRAY. UPSIZES FIRST IF NEEDED
	...
	private void upSize()
	{
		T[] biggerArray = (T[]) new Object[2 * theArray.length];
	...
	{
		ArrBag<T> interectResult = new ArrBag<T>(Math.min(this.size(),other.size())); // change the 0 to the right value for intersect
		
		for (int i = 0; (i < this.size()); i++)
		{
			if ((!interectResult.contains(this.get(i)) && (other.contains(this.get(i)))))
			{
	...
	public ArrBag<T> difference( ArrBag<T> other )
	{
		...
		for (int i = 0; (i < this.size()); i++)
		{
			if ((!diffResult.contains(this.get(i))) && (!other.contains(this.get(i))))
			{
