...
class BSTreeP5<T>
{
	...
	private int countInteriorNodes( BSTNode root)
	{
		...
		return count + countInteriorNodes(root.left) + countInteriorNodes(root.right);
	}
	...
	private int countLevels( BSTNode root)
	{
		...
		return Math.max(countLevels(root.left), countLevels(root.right)) + 1;
	}
	...
	public BSTreeP5(  BSTreeP5<T> other )
	{
		if(other.root == null){
			...
			root = new BSTNode(other.root.key, null, null);
		}
		root.right = BSTreeP5Helper(other.root.right);
		root.left = BSTreeP5Helper(other.root.left);
	}
	private BSTNode<T> BSTreeP5Helper(BSTNode<T> copy){
		if(copy == null){
			return null;
		}
		BSTNode<T> root = new BSTNode(copy.key, null, null);
			root.right = BSTreeP5Helper(copy.right);
			root.left = BSTreeP5Helper(copy.left);
			return root;
		}
